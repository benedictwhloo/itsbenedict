...
export default function Experience() {
  return (
    <section id="experience" className="mx-auto max-w-5xl px-6 py-28">
      ...
      <ol className="mt-12 space-y-10 border-l border-white/10">
        {experience.map((entry) => (
          <li key={entry.role + entry.org} className="relative pl-8">
            <span className="absolute -left-[5px] top-1.5 h-2.5 w-2.5 rounded-full bg-sky-300" />
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <h3 className="text-lg font-medium">{entry.role}</h3>
              <span className="text-sm text-white/40">{entry.period}</span>
            </div>
            <p className="text-sm text-white/50">{entry.org}</p>
            <p className="mt-2 max-w-2xl text-white/70">{entry.description}</p>
          </li>
        ))}
      </ol>
    </section>
