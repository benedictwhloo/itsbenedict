...
export default function Experience() {
  return (
    <section id="experience" className="mx-auto max-w-5xl px-6 py-28">
      ...
      <ol className="mt-12 space-y-10 border-l border-foreground/10">
        {experience.map((entry) => (
          <li key={entry.role + entry.org} className="relative pl-8">
            ...
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              ...
              <span className="text-sm text-foreground/40">{entry.period}</span>
            </div>
            <p className="text-sm text-foreground/50">{entry.org}</p>
            <p className="mt-2 max-w-2xl text-foreground/70">{entry.description}</p>
          </li>
        ))}
      </ol>
    </section>
